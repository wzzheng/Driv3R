...
class BaseStereoViewDataset (EasyDataset):
    ...
    def _crop_resize_if_necessary(self, image, depthmap, intrinsics, resolution, split, rng=None, info=None):
        """ This function:
        ...
        scaling_factor = output_height / h
        
        target_height = output_height
        target_width = int(w * scaling_factor)

        image = image.resize((target_width, target_height), PIL.Image.Resampling.LANCZOS)
        depthmap = cv2.resize(depthmap, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4)
        
        if split == 'left' or split == 'l':
            image = image.crop((0, 0, output_width, output_height))
            depthmap = depthmap[:, :output_width]
            fx = intrinsics[0, 0] * scaling_factor
            fy = intrinsics[1, 1] * scaling_factor
            cx = intrinsics[0, 2] * scaling_factor
            cy = intrinsics[1, 2] * scaling_factor
